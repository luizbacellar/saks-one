...
st.set_page_config(page_title="GATE", layout="centered")
...
if st.session_state.pagina == "home":
    ...
    st.text_input("", "Faça uma pergunta ao GATE", disabled=True)
...
# Página Advisor+
    ...
    if "advisor_chat" not in st.session_state:
        st.session_state.advisor_chat = [
            {"role": "system", "content": "Você é um advisor do GATE especializado em mercados financeiros internacionais. Sempre responda com foco em produtos, teses, estratégias ou análises relevantes para investimentos globais. Se a pergunta fugir do contexto de investimentos internacionais, informe educadamente que está fora do escopo do GATE."}
        ]
        ...
        with st.spinner("Consultando inteligência financeira..."):
            try:
                response = openai.chat.completions.create(
                    model="o4-mini",
                    messages=[
                        {"role": "system", "content": "Você está no módulo INSIGHT do GATE. Responda com foco em explicação técnica, estrutura de produtos e tese de investimento internacional. Se o usuário pedir uma recomendação, direcione para o módulo Advisor+. Se fugir de investimentos internacionais, diga que está fora do escopo do GATE."},
        ...
        with st.spinner("Calculando cenários e risco global..."):
            try:
                response = openai.chat.completions.create(
                    model="o4-mini",
                    messages=[
                        {"role": "system", "content": "Você está no módulo RADAR do GATE. Foque sua resposta na avaliação de risco, exposição setorial, geográfica ou cambial em investimentos internacionais. Se a pergunta for sobre recomendação, envie para Advisor+. Se fugir do contexto de risco global, informe que está fora do escopo."},
...
# Página Pulse One
    ...
    if st.button("🔍 Consultar Pulse IA"):
        with st.spinner("Consultando central de atendimento do GATE..."):
            try:
                response = openai.chat.completions.create(
                    model="o4-mini",
                    messages=[
                        {"role": "system", "content": "Você está no módulo PULSE ONE do GATE. Responda como um atendente de alto nível que resolve dúvidas, esclarece prazos e orienta clientes internacionais. Se for sobre produtos, envie para Advisor+. Se fugir de atendimento financeiro internacional, informe que não se aplica."},
        ...
        with st.spinner("Preparando material educativo e simulações..."):
            try:
                response = openai.chat.completions.create(
                    model="o4-mini",
                    messages=[
                        {"role": "system", "content": "Você está no módulo ACADEMY do GATE. Ensine conceitos sobre produtos financeiros internacionais, simule pitches e ajude o usuário a aprender sobre finanças globais. Se pedirem recomendação ou avaliação, oriente usar Advisor+ ou Insight. Fora desse contexto, diga que está fora do escopo."},
